...
ifeq (Darwin,$(UNAME_S))
	CXX := clang++  # llvm clang, for support of -fsanitize
else
	CXX := g++
endif
...
CXXFLAGS_DEBUG    := -std=c++11 -g -Wall -Wextra -Wpedantic 
ifeq (Linux,$(UNAME_S))
	CXXFLAGS_DEBUG  += -fsanitize=address,leak,undefined
...
# section: files
SRC_FILES       := $(filter-out $(SRC_DIR)/tmp_%.cpp,$(wildcard $(SRC_DIR)/*.cpp))
...
# section: automatic dependency graph generation
CXXFLAGS          += -MMD -MP
CXXOBJFLAGS       += -MMD -MP
